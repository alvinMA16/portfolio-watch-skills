...
analyst revisions, company catalysts, or trading recommendations.
...
- One row per holding with 1D move, relative move versus SPY, volume ratio, and
  portfolio impact.
- The top one or two evidence-backed changes worth inspecting, shown once as
  `值得留意的变化`.
...
- Portfolio Trend: chart evidence for portfolio, holdings, SPY, and QQQ when
  available.
- Worth-reading changes: ranked events that deserve inspection, shown in the
  first-screen region rather than repeated below the chart.
- Evidence explanation: a table showing what was observed and what is not
  wired.
