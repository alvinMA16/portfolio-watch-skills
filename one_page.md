...
Skill 则是一个“方法论 + 操作路由 + 约束 + 可选模板/代码”的包，用来让 agent 在一系列构建契约下，稳定地产出某类结果。

对我们这个 task 中的 portfolio-watch-skill 来说，定义会更窄一些：

  > 一个专门让 agent 从任意用户持仓输入出发，生成 Portfolio Watch Playbook 的 reusable capability，包括监控维度、信号排序、feed/playbook/
  > alert 架构、UI 模板、alert contract 和验证 checklist。
